...
|--------|-------------|---------|
| `--output` | Output directory for key files | `./keys` |
| `--key-size` | RSA key size in bits | `4096` |

Creates `private-key.pem` (keep secure) and `public-key.pem` in the output directory.
